...
def normalized_stacked_bar_plot(adata, x_value, color_value, palette=None, legend=True,ax=None,x_order=None,color_order=None, log=False):
    ...
    if ax is not None:
        normed_tmp.plot(kind='bar',stacked=True,  colormap=palette, ax=ax,edgecolor='w')
    else:
...
def normalized_stacked_bar_plot_from_df(df, x_value, color_value, palette=None,color_uns=None,ax=None):
    ...
    tmp = df.groupby([x_value,color_value])[color_value].count().unstack(color_value).fillna(0)
    print(tmp.divide(tmp.sum(axis=1), axis=0))
    if ax is not None:
...
def grouped_dotplot(x_condition,y_condition,genes,adata,ordered_y_condition=[],within_gene_sep = .6,
    ...
    if scipy.sparse.issparse(adata.raw.X):
        for G in genes:
            ...
            tmp_obs[G] = adata.raw[:,G].X.todense()
            tmp_obs[G+" on"] = adata.raw[:,G].X.todense() > 0
    else:
...
def boxplot_sample_proportions(adata, x_value, color_value,hue="treatment",figsize=(10,5), plottype="box",order=None,hue_order=None,edgecolor=False,swap=False):
    ...
    if plottype=="box":
        ...
        p=sns.boxplot(x=color_value, y=x_value+"_proportion", hue=hue, data=props_df, palette=color_dict,hue_order=hue_order,linewidth=3,order=order)
        ...
        sns.swarmplot(x=color_value, y=x_value+"_proportion", hue=hue, data=props_df,  dodge=True,palette=swarm_palette,edgecolor="white",linewidth=.7,size=3.2, hue_order=hue_order,order=order)
